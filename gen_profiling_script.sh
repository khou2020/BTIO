...
if [[ "$HOSTNAME_PREFIX" == "cori" ]]; then
    ...
    OUTPATH_ROOT=/global/cscratch1/sd/khl7265/FS_64_1M/btio
    ...
    OUTPATH_ROOT=/lus-projects/CSC250STDM12/khou/FS_48_1M/btio
    INFILE=/global/cscratch1/sd/wkliao/FS_1M_32/ND_1951_256K.1.h5
    ACC=CSC250STDM12
    SCRIPT=theta
    QUEUE=default
    ...
    LOGVOL_SRC_PATH=${HOME}/Desktop/logvol/master
    LOGVOL_LIB_PATH=${HOME}/.local/log_io_vol/master
    OUTPATH_ROOT=${HOME}/Desktop/btio/output
    INFILE=${HOME}/Desktop/btio/neardet_r00012096_s01_t00_R19-02-23-miniprod5.i_v1_data.h5caf.h5
    ACC=test_acc
    SCRIPT=theta
    NODE_TYPE=local
    RTL=test_rtl
...
BTIO_HASH=$(git --git-dir .git rev-parse HEAD)
BTIO_HASH_PREFIX=${BTIO_HASH:0:5}
BTIO_DATE=$(stat -c %Y ./btio)
...
FNAME="${XL}_${YL}_${ZL}_${RECS}_logvl${LOGVOL_HASH_PREFIX}_btio${BTIO_HASH_PREFIX}_hdf${HDF5_HASH_PREFIX}_pnc${PNC_HASH_PREFIX}_${HOSTNAME_PREFIX}_${NN}_${PPN}"

SUBMIT_DATE=$(date)

GEN_SCRIPT="m4 -D EXP_NAME=\"${FNAME}\" -D EXP_NN=\"${NN}\" -D EXP_PPN=\"${PPN}\" -D EXP_TL=\"${TL}\" -D EXP_ACC=\"${ACC}\" -D EXP_NODE_TYPE=\"${NODE_TYPE}\" \
-D EXP_OUTDIR_ROOT=\"${OUTPATH_ROOT}\" -D EXP_INFILE=\"${INFILE}\" -D EXP_OP=\"${OP}\" -D EXP_XL=\"${XL}\" -D EXP_ZL=\"${ZL}\" -D EXP_YL=\"${YL}\" -D EXP_RECS=\"${RECS}\" \
-D EXP_RUNS=\"${RUNS}\" -D EXP_RTL=\"${RTL}\" -D EXP_SUBMIT_DATE=\"${SUBMIT_DATE}\"  -D EXP_QUEUE=\"${QUEUE}\" \
-D EXP_BTIO_HASH=\"${BTIO_HASH}\" -D EXP_BTIO_DATE=\"${BTIO_DATE}\" \
